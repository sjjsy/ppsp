...
def test_compose_chain_with_tmo():
    spec = ChainSpec(z_tier="z25", enfuse_id="sel3", tmo_id="fatt", grading_id="dvi2", web=False)
    assert compose_chain(spec) == "z25-sel3-fatt-dvi2"
...
def test_compose_chain_web():
    spec = ChainSpec(z_tier="z100", enfuse_id="sel4", tmo_id="ma06", grading_id="warm", web=True)
    assert compose_chain(spec) == "z100-sel4-ma06-warm-web"


def test_parse_chain_with_tmo():
    filename = "20260416095559-m4azzz-2126-z25-sel3-fatt-dvi2.jpg"
...
def test_parse_chain_web():
    filename = "20260416095559-m4azzz-2126-z100-sel4-ma06-warm-web.jpg"
    spec = parse_chain(filename)
    assert spec is not None
...
def test_parse_chain_original_returns_none():
    # Original camera files have a single collision letter as chain
    filename = "20260416095559-m4azzz-2126-a.arw"
    assert parse_chain(filename) is None


def test_roundtrip_compose_parse():
    spec = ChainSpec(z_tier="z13", enfuse_id="cont", tmo_id="ferw", grading_id="brig", web=False)
    chain = compose_chain(spec)
    # Embed in a realistic filename
