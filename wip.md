...
- CT args prepended to grading args in a single `convert` call after a single
  `-colorspace sRGB`. Leading `-colorspace sRGB` stripped from grading to avoid
  duplication. Implemented in `processing.py:apply_grading()`.

- `warm` and `dv1w` grading presets retired; CT is now a separate chain element.
